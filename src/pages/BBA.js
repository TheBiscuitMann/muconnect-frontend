...
// ✨ FIXED: Added the missing constants!
const NAVY = '#1e2a6e';
const GREY = '#6b7280';
const DEPT_COLOR = '#E65100'; // Sunset Orange

const DEPT_NAME = 'Business Administration (BBA)';
const SCHOOL_NAME = 'School of Business & Economics';

const deptHead = {
  name: "Dr. [Head's Name]",
  title: "Head, Business Administration",
  message: "Welcome to the BBA program. We cultivate visionary leaders and disruptive entrepreneurs. Through case studies, industry collaborations, and rigorous financial analysis, we empower our students to navigate and dominate the complex global business landscape.",
};

const quickLinks = [ "BBA Program Details", "Entrepreneurship Incubator", "Business Case Club", "Internship Placement Office", "Academic Calendar" ];

const notices = [
  { date: '28-MAY', title: 'National Business Case Competition' },
  { date: '18-MAY', title: 'Alumni Mixer: Startup Founders' },
  { date: '10-MAY', title: 'Seminar: Global Supply Chain Logistics' },
];

const contact = {
  office: "Room 201, Business Building",
  phone: "+880-1234-567896",
  email: "head_bba@metrouni.edu.bd",
};
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", color: NAVY, background: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        background: `linear-gradient(to right, ${NAVY}, ${DEPT_COLOR})`, 
          ...
          <div style={{ marginBottom: '56px' }}>
            ...
            <div style={{ display: 'flex', gap: '32px', alignItems: 'flex-start' }}>
              <div style={{ 
                ...
                background: `linear-gradient(135deg, ${DEPT_COLOR}, ${NAVY})`,
                boxShadow: `0 16px 32px ${DEPT_COLOR}30` 
              }}>
              ...
              <div>
                ...
                <div style={{ borderLeft: `4px solid ${DEPT_COLOR}`, paddingLeft: '16px' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '14px', fontWeight: '700', color: DEPT_COLOR, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    {deptHead.title}
                  </p>
                </div>
              </div>
              ...
              {quickLinks.map(link => (
                <li key={link}>
                  <a href="#" style={{ color: NAVY, textDecoration: 'none', fontSize: '15px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '12px', transition: 'color 0.2s' }}
                     onMouseOver={e => e.target.style.color = DEPT_COLOR} onMouseOut={e => e.target.style.color = NAVY}
                  >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {notices.map((notice, i) => (
                <div key={i} style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '15px', color: '#444', fontWeight: '600', lineHeight: 1.5, cursor: 'pointer' }} onMouseOver={e => e.target.style.color = DEPT_COLOR} onMouseOut={e => e.target.style.color = '#444'}>
                    {notice.title}
                  </p>
                </div>
              ))}
            </div>
            <button style={{ marginTop: '24px', background: DEPT_COLOR, color: '#fff', border: 'none', borderRadius: '6px', fontWeight: '700', fontSize: '13px', cursor: 'pointer', padding: '10px 20px', width: '100%' }}>
              View All Notices
            </button>
          </div>
